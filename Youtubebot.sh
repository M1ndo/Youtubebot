...
checkroot() {
if [[ "$(id -u)" -ne 0 ]]; then
	printf "\e[1;77mPlease, run this programm as root!\n\e[0m"
	exit 1 
...
checkroot
if [[ -z $1 ]]; then
printf "\e[1;92m[*] Usage: sudo ./Youtubebot.sh url \e[0m\n"
exit 1	
fi 

dependencies() {
command -v tor > /dev/null 2>&1 || { echo >&2 "I require tor but it's not installed. Run apt-get install tor . Aborting. "; exit; }
command -v proxychains > /dev/null 2>&1 || { echo >&2 "I require proxychains but its not installed. Run apt-get install proxychains. Aborting."; exit 1; }
